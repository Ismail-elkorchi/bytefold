...
type DenoApi = {
  ...
  stat: (path: string | URL) => Promise<{ size: number }>;
...
class DenoFileRandomAccess implements RandomAccess {
  private readonly filePromise: Promise<DenoFile>;
  private readonly sizePromise: Promise<bigint>;
  private readonly deno: DenoApi;

  private constructor(path: string | URL) {
    const deno = requireDeno();
    this.deno = deno;
    this.filePromise = deno.open(path, { read: true });
    this.sizePromise = deno.stat(path).then((stat) => BigInt(stat.size));
  }

  static async fromPath(path: string | URL): Promise<DenoFileRandomAccess> {
    return new DenoFileRandomAccess(path);
  }
  ...
  async read(offset: bigint, length: number, signal?: AbortSignal): Promise<Uint8Array> {
    ...
    if (!file.seek || !file.read) {
      throw new Error('Deno file access is not available in this runtime.');
    }
    const position = Number(offset);
    if (!Number.isFinite(position)) {
      throw new RangeError('Offset exceeds safe integer range');
    }
    await file.seek(position, this.deno.SeekMode.Start);
    const buffer = new Uint8Array(length);
    const bytesRead = await file.read(buffer);
    throwIfAborted(signal);
    if (!bytesRead) return new Uint8Array(0);
...
export async function openArchive(input: DenoArchiveInput, options?: ArchiveOpenOptions): Promise<ArchiveReader> {
  ...
  if (typeof input === 'string' || input instanceof URL) {
    ...
    if (shouldPreflightZip(formatOption, filename)) {
      ...
      const reader = await DenoFileRandomAccess.fromPath(path);
...
function resolveRemoteArchiveUrl(value: string | URL, options?: ArchiveOpenOptions): URL | null {
  ...
  if (url.protocol === 'http:') {
    ...
    throw new ArchiveError(
      ...
      'HTTP URL inputs require { url: { allowHttp: true } } in the Deno adapter'
    );
  }
  return null;
}
