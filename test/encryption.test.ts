...
import { ZipError, ZipReader, ZipWriter } from 'zip-next';

const PASSWORD = 'zip-next-password';
...
test('seekable patch mode writes encrypted entries without data descriptor', async () => {
  ...
  const filePath = path.join(tmpdir(), `zip-next-aes-seekable-${Date.now()}.zip`);
...
async function makeTempDir(): Promise<string> {
  const dir = path.join(tmpdir(), `zip-next-7z-${Math.random().toString(16).slice(2)}`);
