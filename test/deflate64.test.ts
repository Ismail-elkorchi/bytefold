...
function prngBytes(length: number): Uint8Array {
  let seed = 0x12345678;
...
test('deflate64 compatibility subset (deflate stream wrapped as method 9)', async () => {
  const data = prngBytes(8192);
...
test('deflate64 distance > 32KiB is supported', async () => {
  const deflate64Stream = buildDeflate64DistanceStream();
  const output = new Uint8Array(40003).fill(0x41);
  const zip = buildZip([
    {
      name: 'distance.bin',
      method: 9,
      compressed: deflate64Stream,
      uncompressed: output
    }
  ]);
  ...
  const stream = await reader.open(entry);
  const buf = await new Response(stream).arrayBuffer();
  assert.deepEqual(new Uint8Array(buf), output);
});

test('mixed methods route through codec registry', async (t) => {
  if (typeof zlib.createZstdCompress !== 'function') {
    t.skip('zstd not available');
    return;
  }
  const storeData = new TextEncoder().encode('store');
  const deflateData = new TextEncoder().encode('deflate');
  const deflate64Data = new TextEncoder().encode('deflate64');
  const zstdData = new TextEncoder().encode('zstd');

  const deflateCompressed = zlib.deflateRawSync(deflateData);
  const deflate64Compressed = zlib.deflateRawSync(deflate64Data);
  const zstdCompressed = await new Promise<Uint8Array>((resolve, reject) => {
  ...
  const zip = buildZip([
    { name: 'store.txt', method: 0, compressed: storeData, uncompressed: storeData },
    { name: 'deflate.txt', method: 8, compressed: deflateCompressed, uncompressed: deflateData },
    { name: 'deflate64.txt', method: 9, compressed: deflate64Compressed, uncompressed: deflate64Data },
    { name: 'zstd.txt', method: 93, compressed: zstdCompressed, uncompressed: zstdData }
  ]);
  ...
  assert.deepEqual(await readEntry('store.txt'), storeData);
  assert.deepEqual(await readEntry('deflate.txt'), deflateData);
  assert.deepEqual(await readEntry('deflate64.txt'), deflate64Data);
  assert.deepEqual(await readEntry('zstd.txt'), zstdData);
});
