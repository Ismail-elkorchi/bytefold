...
async function decompressWithChunks(
  ...
  const stream = readable.pipeThrough(transform) as ReadableStream<Uint8Array>;
...
async function startRangeServer(
  ...
  const serve = async () => {
    const serveHttp = (Deno as unknown as {
      serveHttp: (conn: unknown) => AsyncIterable<{
        request: Request;
        respondWith: (response: Response) => Promise<void>;
      }>;
    }).serveHttp;
    for await (const conn of listener) {
      const httpConn = serveHttp(conn) as AsyncIterable<{
        request: Request;
        respondWith: (response: Response) => Promise<void>;
      }> & { close?: () => void };
      connections.add(httpConn as { close: () => void });
      void (async () => {
        try {
          for await (const event of httpConn) {
            ...
            if (request.method === 'HEAD') {
              ...
              await event.respondWith(new Response(null, { headers }));
              continue;
            }
            ...
            if ((mode === 'range' || mode === 'etag-mismatch') && range) {
              ...
              if (!match) {
                await event.respondWith(new Response(null, { status: 416 }));
                continue;
              }
            ...
            await event.respondWith(
              new Response(payload, { status: 200, headers: { 'content-length': String(data.length) } })
            );
          }
        } finally {
          connections.delete(httpConn as { close: () => void });
          httpConn.close?.();
        }
      })();
    }
  ...
  void serve();
  return {
    url: `http://127.0.0.1:${addr.port}/${name}`,
    stats,
    close: () => {
      listener.close();
            ...
            for await (const event of httpConn) {
              ...
              const respond = async (response: Response) => {
                try {
                  ...
                  // ignore cancellations for aborted connections
                }
              };
              ...
              if (request.method === 'HEAD') {
                ...
                if (options.mode === 'head-blocked') {
                  ...
                  await respond(new Response(null, { status: 405 }));
                  continue;
                }
                stats.statuses.push(200);
                const headers = new Headers({ 'content-length': String(data.length) });
                addValidators(headers);
                await respond(new Response(null, { headers }));
                continue;
              }
              ...
              if (options.mode === 'if-range-200') {
                ...
                if (range && ifRange && currentEtag && ifRange !== currentEtag) {
                  ...
                  addValidators(headers);
                  await respond(
                    new Response(chunkedBody(data, stats, { trackRangeBytes: false }), { status: 200, headers })
                  );
                  continue;
                }
              }
              ...
              if (!match) {
                ...
                await respond(new Response(null, { status: 416 }));
                continue;
              }
              const start = Number(match[1]);
              const end = match[2] ? Number(match[2]) : data.length - 1;
              ...
              if (options.mode === 'short-body') {
                ...
                await respond(
                  new Response(chunkedBody(truncated, stats, { trackRangeBytes: true }), { status: 206, headers })
                );
                continue;
              }
              ...
              if (options.mode === 'content-encoding-slow') {
                await respond(
                  new Response(chunkedBody(body, stats, { trackRangeBytes: true, chunkSize: 512, delay: true }), {
                    status: 206,
                    headers
                  })
                );
              ...
              headers.set('content-length', String(body.length));
              await respond(new Response(chunkedBody(body, stats, { trackRangeBytes: true }), { status: 206, headers }));
            }
          } catch (error) {
            if (!isExpectedConnectionAbortError(error)) throw error;
          }
        } finally {
          connections.delete(httpConn as { close: () => void });
          httpConn.close?.();
...
function isExpectedConnectionAbortError(error: unknown): boolean {
  if (!(error instanceof Error)) return false;
  const message = error.message.toLowerCase();
  return message.includes('user body write aborted') || message.includes('early end');
}
...
Deno.test('deno smoke: zip, tar, tgz', async () => {
  ...
  if (!tarEntries.includes('greet.txt') || !sawTar) throw new Error('tar entry missing');
  ...
  const tgzReader = await openArchive(tgzOutput);
  if (tgzReader.format !== 'tgz') throw new Error('tgz writer output not detected');
  const tgzEntries: Record<string, Uint8Array> = {};
  for await (const entry of tgzReader.entries()) {
  ...
  const requiredAlgorithms: Array<'gzip' | 'deflate-raw' | 'deflate'> = ['gzip', 'deflate-raw', 'deflate'];
  for (const algorithm of requiredAlgorithms) {
    const support = caps.algorithms[algorithm];
    if (!support.compress || !support.decompress) {
      throw new Error(`Deno runtime missing ${algorithm} support; requires ${versions.deno ?? '>=1.23.0'}`);
    }
  }
  ...
  const expectedHello = new TextDecoder().decode(
    await Deno.readFile(new URL('../test/fixtures/expected/hello.txt', import.meta.url))
  );
  ...
  await gzWriter.add('hello.txt', encoder.encode(expectedHello));
  await gzWriter.close();
  const gzOutput = concatChunks(gzChunks);
  const gzWriterReader = await openArchive(gzOutput, { filename: 'hello.txt.gz' });
  if (gzWriterReader.format !== 'gz') throw new Error('gz writer output not detected');
  let sawGzWriter = false;
  for await (const entry of gzWriterReader.entries()) {
    const data = await collect(await entry.open());
    const text = new TextDecoder().decode(data);
    if (text !== expectedHello) throw new Error('gz writer content mismatch');
    sawGzWriter = true;
  }
  if (!sawGzWriter) throw new Error('gz writer missing entry');
  const gzBytes = await Deno.readFile(new URL('../test/fixtures/hello.txt.gz', import.meta.url));
  ...
  if (!sawGz) throw new Error('gz missing entry');

  const bz2Bytes = await Deno.readFile(new URL('../test/fixtures/hello.txt.bz2', import.meta.url));
  ...
  if (!sawBz2) throw new Error('bz2 missing entry');
  ...
  for (const { fixture, expected, expectedRoot } of bcjFixtures) {
    const bytes = await Deno.readFile(new URL(`../test/fixtures/xz-bcj/${fixture}`, import.meta.url));
  ...
  {
    const thirdZipBytes = await Deno.readFile(
  ...
  {
    const thirdTarBytes = await Deno.readFile(
      new URL('../test/fixtures/thirdparty/tar/pax.tar', import.meta.url)
    );
  ...
  {
    const gzipHeaderBytes = await Deno.readFile(
      new URL('../test/fixtures/gzip-header-options.gz', import.meta.url)
    );
    const gzipExpected = await Deno.readFile(
      new URL('../test/fixtures/expected/hello.txt', import.meta.url)
    );
  ...
  {
    const fhcrcOk = await Deno.readFile(new URL('../test/fixtures/gzip-fhcrc-ok.gz', import.meta.url));
    const fhcrcExpected = await Deno.readFile(new URL('../test/fixtures/expected/hello.txt', import.meta.url));
    const okArchive = await openArchive(fhcrcOk, { format: 'gz' });
    ...
    const fhcrcBad = await Deno.readFile(new URL('../test/fixtures/gzip-fhcrc-bad.gz', import.meta.url));
    const errorSchema = (JSON.parse(
      new TextDecoder().decode(await Deno.readFile(new URL('../schemas/error.schema.json', import.meta.url)))
    ) as unknown) as JsonSchema;
    const { validateSchema } = (await import(new URL('./schema-validator.ts', import.meta.url).href)) as {
      validateSchema: (schema: JsonSchema, value: unknown) => { ok: boolean; errors: string[] };
    };
    let sawError = false;
    try {
      await openArchive(fhcrcBad, { format: 'gz' });
    } catch (err) {
  ...
  {
    const mixedBytes = await Deno.readFile(new URL('../test/fixtures/xz-mixed/delta-x86-lzma2.xz', import.meta.url));
    const mixedExpected = await Deno.readFile(
  ...
  {
    const concatBytes = await Deno.readFile(
      new URL('../test/fixtures/xz-concat/concat-two-streams-bcj.xz', import.meta.url)
    );
    const concatExpected = await Deno.readFile(
  ...
  {
    const chunkBytes = await Deno.readFile(new URL('../test/fixtures/xz-bcj/x86.xz', import.meta.url));
    const chunkExpected = await Deno.readFile(new URL('../test/fixtures/expected/xz-bcj-x86.bin', import.meta.url));
  ...
  const xzShaBytes = await Deno.readFile(new URL('../test/fixtures/xz-check-sha256.xz', import.meta.url));
  const xzShaExpected = await Deno.readFile(new URL('../test/fixtures/expected/xz-check-sha256.bin', import.meta.url));
  ...
  const detectionSchema = (JSON.parse(
    new TextDecoder().decode(await Deno.readFile(new URL('../schemas/detection-report.schema.json', import.meta.url)))
  ) as unknown) as JsonSchema;
  const auditSchema = (JSON.parse(
    new TextDecoder().decode(await Deno.readFile(new URL('../schemas/audit-report.schema.json', import.meta.url)))
  ) as unknown) as JsonSchema;
  const normalizeSchema = (JSON.parse(
    new TextDecoder().decode(await Deno.readFile(new URL('../schemas/normalize-report.schema.json', import.meta.url)))
  ) as unknown) as JsonSchema;
  const capabilitiesSchema = (JSON.parse(
    new TextDecoder().decode(await Deno.readFile(new URL('../schemas/capabilities-report.schema.json', import.meta.url)))
  ) as unknown) as JsonSchema;
  const { validateSchema } = (await import(new URL('./schema-validator.ts', import.meta.url).href)) as {
    validateSchema: (schema: JsonSchema, value: unknown) => { ok: boolean; errors: string[] };
  };

  const tarZstBytes = await Deno.readFile(new URL('../test/fixtures/fixture.tar.zst', import.meta.url));
  const tarBrBytes = await Deno.readFile(new URL('../test/fixtures/fixture.tar.br', import.meta.url));
  await assertUnsupportedFormat('tar.zst', tarZstBytes, 'COMPRESSION_UNSUPPORTED_ALGORITHM', errorSchema, validateSchema);
  await assertUnsupportedFormat('tar.br', tarBrBytes, 'COMPRESSION_UNSUPPORTED_ALGORITHM', errorSchema, validateSchema);

  const brBytes = await Deno.readFile(new URL('../test/fixtures/hello.txt.br', import.meta.url));
  const zstBytes = await Deno.readFile(new URL('../test/fixtures/hello.txt.zst', import.meta.url));
  await assertUnsupportedFormat('br', brBytes, 'COMPRESSION_UNSUPPORTED_ALGORITHM', errorSchema, validateSchema);
  await assertUnsupportedFormat('zst', zstBytes, 'COMPRESSION_UNSUPPORTED_ALGORITHM', errorSchema, validateSchema);

  await assertWriterUnsupportedFormat('tar.br', errorSchema, validateSchema);
  await assertWriterUnsupportedFormat('br', errorSchema, validateSchema);
  await assertWriterUnsupportedFormat('tar.zst', errorSchema, validateSchema);
  await assertWriterUnsupportedFormat('zst', errorSchema, validateSchema);
  ...
  await closeArchive(zipArchive);

  const tarBz2ArchiveForNormalize = await openArchive(tarBz2Bytes);
  await assertAuditNormalizeRoundtrip(tarBz2ArchiveForNormalize, auditSchema, normalizeSchema, validateSchema);

  const txzArchiveForNormalize = await openArchive(txzBytes);
  await assertAuditNormalizeRoundtrip(txzArchiveForNormalize, auditSchema, normalizeSchema, validateSchema);
  ...
  await assertSingleFileAuditNormalize(xzArchive, auditSchema, errorSchema, validateSchema);

  {
    const bytes = await Deno.readFile(new URL('../test/fixtures/hello.txt.bz2', import.meta.url));
    let error: unknown;
    try {
    ...
    if (!result.ok) throw new Error(`bzip2 resource error schema failed: ${result.errors.join('\\n')}`);
  ...
  {
    const bytes = await Deno.readFile(new URL('../test/fixtures/concat-limit.bz2', import.meta.url));
    let error: unknown;
    try {
    ...
    const json = error.toJSON() as { context?: Record<string, string> };
    const result = validateSchema(errorSchema, json);
    if (!result.ok) throw new Error(`concat bzip2 resource error schema failed: ${result.errors.join('\\n')}`);
    if (!json.context?.requiredBlockSize || !json.context?.limitBlockSize) {
    ...
    const preflightIssue = report.issues.find(
      (issue: { code?: string }) => issue.code === 'COMPRESSION_RESOURCE_PREFLIGHT_INCOMPLETE'
    );
    if (!preflightIssue) throw new Error('missing bzip2 preflight incomplete issue');
  }

  {
    const bytes = await Deno.readFile(new URL('../test/fixtures/hello.txt.xz', import.meta.url));
    let error: unknown;
    try {
    ...
    const json = error.toJSON() as { context?: Record<string, string> };
    const result = validateSchema(errorSchema, json);
    if (!result.ok) throw new Error(`xz resource error schema failed: ${result.errors.join('\\n')}`);
  ...
  {
    const bytes = await Deno.readFile(new URL('../test/fixtures/xz-check-sha256.xz', import.meta.url));
    const expected = await Deno.readFile(new URL('../test/fixtures/expected/xz-check-sha256.bin', import.meta.url));
    ...
    const json = error.toJSON() as { context?: Record<string, string> };
    const result = validateSchema(errorSchema, json);
    if (!result.ok) throw new Error(`xz sha256 error schema failed: ${result.errors.join('\\n')}`);
    if (!json.context?.check) throw new Error('xz sha256 error missing context');
  }

  {
    const bytes = await Deno.readFile(new URL('../test/fixtures/concat-limit.xz', import.meta.url));
    ...
    const json = error.toJSON() as { context?: Record<string, string> };
    const result = validateSchema(errorSchema, json);
    if (!result.ok) throw new Error(`concat xz resource error schema failed: ${result.errors.join('\\n')}`);
    if (!json.context?.requiredDictionaryBytes || !json.context?.limitDictionaryBytes) {
    ...
    const issue = report.issues.find((entry: { code?: string }) => entry.code === 'COMPRESSION_RESOURCE_LIMIT');
  ...
  {
    const bytes = await Deno.readFile(new URL('../test/fixtures/xz-dict-huge.xz', import.meta.url));
  ...
  {
    const bytes = await Deno.readFile(new URL('../test/fixtures/zip-preflight/basic.zip', import.meta.url));
...

  {
    ...
    const bigZipPath = `${tmp}/zip-url-big.zip`;
    ...
    const bigZipBytes = await Deno.readFile(bigZipPath);
    ...
    try {
      ...
      serverBadRange.close();
    }

    const serverShortBody = await startZipRangeServer(bigZipBytes, 'zip-url-big.zip', {
      mode: 'short-body',
      etag: ZIP_ETAG_V1,
      lastModified: ZIP_LAST_MODIFIED
    });
    try {
      let error: unknown;
      try {
        const reader = await openArchive(serverShortBody.url, { format: 'zip' });
        try {
      ...
      if (!(error instanceof ZipError) || error.code !== 'ZIP_HTTP_BAD_RESPONSE') {
        throw new Error('zip url short-body did not throw ZIP_HTTP_BAD_RESPONSE');
      }
      const json = error.toJSON() as { code?: string };
      const result = validateSchema(errorSchema, json);
      if (!result.ok) throw new Error(`zip url short-body schema failed: ${result.errors.join('\\n')}`);
    } finally {
      serverShortBody.close();
    }
...

  {
    ...
    try {
      ...
      const incomplete = report.issues.find((entry: { code?: string }) => entry.code === 'COMPRESSION_RESOURCE_PREFLIGHT_INCOMPLETE');
  ...
  {
    const bytes = await Deno.readFile(new URL('../test/fixtures/xz-many-blocks/many-blocks.xz', import.meta.url));
    const expected = await Deno.readFile(new URL('../test/fixtures/xz-many-blocks/many-blocks.bin', import.meta.url));
  ...
  const expectedIssues = (JSON.parse(
    new TextDecoder().decode(await Deno.readFile(new URL('expected-issues.json', ambiguousRoot)))
  ...
  for (const [fixtureName, expected] of Object.entries(expectedIssues)) {
    const bytes = await Deno.readFile(new URL(fixtureName, ambiguousRoot));
    ...
    if (!auditResult.ok) throw new Error(`ambiguous audit schema failed: ${auditResult.errors.join('\\n')}`);
    ...
    if (fixtureName === 'zip-casefold-fuss.zip' || fixtureName === 'zip-casefold-sigma.zip') {
      const issue = audit.issues.find(
        (candidate: { code: string; details?: unknown }) => candidate.code === 'ZIP_CASE_COLLISION'
      );
    ...
    if (fixtureName === 'zip-casefold-turkic.zip') {
      if (audit.issues.some((candidate: { code: string }) => candidate.code === 'ZIP_CASE_COLLISION')) {
    ...
    if ('output' in expectation) {
      ...
      const expectedBytes = await Deno.readFile(new URL(expectation.output, ambiguousRoot));
  ...
  {
    const bytes = await Deno.readFile(new URL('zip-casefold-turkic.zip', ambiguousRoot));
    const reader = await openArchive(bytes, { filename: 'zip-casefold-turkic.zip' });
    await normalizeToBytes(reader);
  }

  const concatGzBytes = await Deno.readFile(new URL('../test/fixtures/concat.gz', import.meta.url));
  ...
  if (!sawConcatGz) throw new Error('concat gz missing entry');

  const concatBz2Bytes = await Deno.readFile(new URL('../test/fixtures/concat.bz2', import.meta.url));
  ...
  if (!sawConcat) throw new Error('concat bz2 missing entry');
...
async function assertAuditNormalizeRoundtrip(
  ...
  const audit = await reader.audit({ profile: 'agent' });
  const auditResult = validateSchema(auditSchema, toJson(audit));
  if (!auditResult.ok) throw new Error(`audit schema validation failed: ${auditResult.errors.join('\\n')}`);
  if (!(audit as { ok?: boolean }).ok) throw new Error('audit report not ok');

  const normalized = await normalizeToBytes(reader);
  const normalizeResult = validateSchema(normalizeSchema, toJson(normalized.report));
  if (!normalizeResult.ok) throw new Error(`normalize schema validation failed: ${normalizeResult.errors.join('\\n')}`);
  if (!(normalized.report as { ok?: boolean }).ok) throw new Error('normalize report not ok');
  ...
  if (!audit2Result.ok) throw new Error(`normalized audit schema validation failed: ${audit2Result.errors.join('\\n')}`);
  if (!(audit2 as { ok?: boolean }).ok) throw new Error('normalized audit report not ok');
}
...
async function assertSingleFileAuditNormalize(
  ...
  const audit = await reader.audit({ profile: 'agent' });
  const auditResult = validateSchema(auditSchema, toJson(audit));
  if (!auditResult.ok) throw new Error(`single-file audit schema validation failed: ${auditResult.errors.join('\\n')}`);
  if (!(audit as { ok?: boolean }).ok) throw new Error('single-file audit report not ok');
...
async function assertUnsupportedFormat(
  format: 'tar.zst' | 'tar.br' | 'br' | 'zst',
  data: Uint8Array,
  expectedCode: 'COMPRESSION_UNSUPPORTED_ALGORITHM',
  schema: JsonSchema,
  validateSchema: (schema: JsonSchema, value: unknown) => { ok: boolean; errors: string[] }
): Promise<void> {
  let error: unknown;
  try {
    await openArchive(data, { format });
  } catch (err) {
    error = err;
  }
  if (!(error instanceof CompressionError)) {
    throw new Error(`expected CompressionError for ${format}`);
  }
  if (error.code !== expectedCode) {
    throw new Error(`expected ${expectedCode} for ${format}`);
  }
  const json = error.toJSON();
...
async function assertWriterUnsupportedFormat(
  format: 'tar.br' | 'br' | 'tar.zst' | 'zst',
  schema: JsonSchema,
  validateSchema: (schema: JsonSchema, value: unknown) => { ok: boolean; errors: string[] }
): Promise<void> {
  let error: unknown;
  ...
  if (!(error instanceof CompressionError)) {
    throw new Error(`expected CompressionError for ${format} writer`);
  }
  if (error.code !== 'COMPRESSION_UNSUPPORTED_ALGORITHM') {
    throw new Error(`expected COMPRESSION_UNSUPPORTED_ALGORITHM for ${format} writer`);
  }
  const json = error.toJSON();
  const result = validateSchema(schema, json);
  if (!result.ok) {
