...
| Complaint | Source | zip-next response | Proof |
| --- | --- | --- | --- |
...
| ZIP64 hang on extract | https://github.com/ZJONSSON/node-unzipper/issues/358 | AbortSignal support and streaming read | `test/abort-progress.test.ts` |
...
| Corrupt ZIP outputs | https://github.com/cthackers/adm-zip/issues/282 | Spec-first writer + CRC validation | `test/zip.test.ts`, `test/zip.test.ts` |
...
| AES out of scope | https://github.com/antelle/node-stream-zip#limitations | AES read/write supported | `test/encryption.test.ts`, `scripts/interop.mjs` |
