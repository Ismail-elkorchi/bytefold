...
15. Decompression output ceilings enforce `maxTotalDecompressedBytes` for gzip/deflate/brotli/zstd and fail with `COMPRESSION_RESOURCE_LIMIT` without emitting beyond the limit. High-level gzip `openArchive(...)` honors both output ceilings, and ZIP/TAR readers reject entry-count, EOCD-comment, uncompressed-size, total-uncompressed-size, and compression-ratio overflows with typed errors. (tests: `test/compression-resource-limits.test.ts`, `test/archive-resource-limit-proofs.test.ts`)
