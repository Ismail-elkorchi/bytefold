...
15. Decompression ceilings enforce `maxTotalDecompressedBytes` for gzip/deflate/brotli/zstd and `maxCompressionRatio` for runtime-backed gzip/deflate/brotli/zstd, failing with typed `COMPRESSION_RESOURCE_LIMIT` without emitting beyond the configured ceiling. High-level gzip/tgz `openArchive(...)` resolves profile-derived decompression ceilings before inflate and preserves explicit per-knob overrides, while ZIP/TAR readers reject entry-count, EOCD-comment, uncompressed-size, total-uncompressed-size, and compression-ratio overflows with typed errors. (tests: `test/compression-resource-limits.test.ts`, `test/archive-resource-limit-proofs.test.ts`, `test/option-precedence.test.ts`, `test/bun.smoke.ts`, `test/deno.smoke.ts`)
...
34. `extractAll` blocks path traversal. When `shouldAllowSymlinks` is enabled, Node ZIP extraction still rejects symlink targets or follow-on writes that would escape the destination root while preserving contained symlink extraction across the Node platform matrix. Deterministic ZIP/TAR adversarial corpora also surface traversal as audit failures (`ZIP_PATH_TRAVERSAL` / `TAR_PATH_TRAVERSAL`) and fail `assertSafe` under the agent profile. (tests: `test/zip.test.ts`, `test/security-audit-simulation.test.ts`)
...
46. Browser smoke verifies the public web/compress entrypoints in real Chromium/Firefox/WebKit: Blob ZIP roundtrip, ZIP/TAR writer roundtrip, capability-gated hostile decompression-ratio rejection, hostile tgz profile-agent rejection, and URL `maxInputBytes` abort remains bounded without HTTP Range requests. (tests: `test/browser/web-entrypoint.pw.ts`)
...
52. Unicode Trojan Source directionality controls are blocked by repository scanning: tracked text files must not contain bidi override/isolation control code points, including files that also contain NUL bytes, and violations fail `npm run check` with file/line/codepoint diagnostics. (tests: `test/unicode-safety-check.test.ts`)
...
55. Node adapter full-fetch non-ZIP file and URL inputs enforce raw input byte ceilings (`maxInputBytes`, `maxTotalDecompressedBytes`, `maxTotalUncompressedBytes`), cancel oversized `Content-Length` response bodies before throwing, and cancel over-limit URL reads with bounded transfer. (tests: `test/node-input-limits.test.ts`)
...
61. Bun and Deno runtime full-fetch non-ZIP URL inputs enforce raw input byte ceilings from `Content-Length` and during streaming reads, cancel oversized `Content-Length` response bodies before throwing, and `TarReader.fromUrl(...)` applies the same URL byte ceilings with typed `RangeError` overflow behavior. (tests: `test/node-input-limits.test.ts`, `test/bun.smoke.ts`, `test/deno.smoke.ts`)
62. Deflate64 bit-reading remains streaming-bounded during active decode: fully consumed compressed chunks are released before close, so retained compressed input stays within a small fixed window instead of growing with total input bytes. (tests: `test/deflate64-retention.test.ts`)
63. Cross-runtime hostile proofs are capability-aware across Node, Bun, Deno, and real browsers: runtime-backed decompressor ratio overflows fail with typed `COMPRESSION_RESOURCE_LIMIT`, gzip/tgz `openArchive(..., { profile: 'agent' })` rejects hostile over-ratio inputs unless the caller explicitly relaxes the ratio ceiling, and browser URL `maxInputBytes` adversarial aborts remain bounded without `Range` requests. (tests: `test/compression-resource-limits.test.ts`, `test/archive-resource-limit-proofs.test.ts`, `test/node-input-limits.test.ts`, `test/bun.smoke.ts`, `test/deno.smoke.ts`, `test/browser/web-entrypoint.pw.ts`, `test/web-adapter.test.ts`)
64. Node remote fetch classification is explicit and fail-closed: only `http://` or `https://` string inputs and `URL` objects reach the remote-fetch branch, while local file path strings and `file:` URLs stay on local reads and never invoke `fetch`. (tests: `test/node-input-limits.test.ts`)
...
- Browser credibility contract: Playwright browser smoke executes the public web/compress entrypoints in real Chromium/Firefox/WebKit and proves Blob ZIP roundtrip, ZIP/TAR writer roundtrip, capability-gated hostile decompression-ratio rejection, hostile tgz profile-agent rejection, and URL `maxInputBytes` adversarial abort behavior (bounded transfer + no `Range` requests). (tests: `test/browser/web-entrypoint.pw.ts`)
...
- Precedence rules (profile vs limits):
  - `openArchive(...)`: `profile` chooses reader defaults for ZIP/TAR readers, and gzip/tgz layered opens resolve profile-derived decompression ceilings before inflate; `limits` overrides only the specified fields and unspecified ceilings stay on profile defaults. (tests: `test/option-precedence.test.ts`, `test/archive-resource-limit-proofs.test.ts`, `test/deno.smoke.ts`, `test/bun.smoke.ts`)
