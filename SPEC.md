...
15. Decompression ceilings enforce `maxTotalDecompressedBytes` for gzip/deflate/brotli/zstd and `maxCompressionRatio` for runtime-backed gzip/deflate/brotli/zstd, failing with typed `COMPRESSION_RESOURCE_LIMIT` without emitting beyond the configured ceiling. High-level gzip `openArchive(...)` honors explicit output ceilings, and ZIP/TAR readers reject entry-count, EOCD-comment, uncompressed-size, total-uncompressed-size, and compression-ratio overflows with typed errors. (tests: `test/compression-resource-limits.test.ts`, `test/archive-resource-limit-proofs.test.ts`, `test/bun.smoke.ts`, `test/deno.smoke.ts`)
