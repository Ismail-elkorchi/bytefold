...
34. `extractAll` blocks path traversal. Node ZIP extraction also rejects symlink targets or follow-on writes that would escape the destination root when symlink materialization is enabled. Deterministic ZIP/TAR adversarial corpora also surface traversal as audit failures (`ZIP_PATH_TRAVERSAL` / `TAR_PATH_TRAVERSAL`) and fail `assertSafe` under the agent profile. (tests: `test/zip.test.ts`, `test/security-audit-simulation.test.ts`)
