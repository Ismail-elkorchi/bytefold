...
const vliRoot = resolve(fixturesRoot, 'xz-vli');
...
const payload = buildPayload(131072);
...
const stream = buildStream([block], CHECK_NONE);

await mkdir(vliRoot, { recursive: true });
await writeFile(resolve(vliRoot, 'vli-uncompressed-128.xz'), stream);
await writeFile(resolve(vliRoot, 'vli-uncompressed-128.bin'), payload);
...
function buildBlock({ payload, encoded, filters, checkType }) {
  ...
  const lzma2Data = encodeLzma2UncompressedChunks(encoded);
...
function encodeLzma2UncompressedChunks(payload) {
  if (payload.length === 0) {
    throw new Error('payload size must be >= 1 for uncompressed chunks');
  }
  const chunks = [];
  let offset = 0;
  while (offset < payload.length) {
    const remaining = payload.length - offset;
    const size = Math.min(remaining, 0x10000);
    const headerSize = size - 1;
    const header = new Uint8Array([0x01, (headerSize >>> 8) & 0xff, headerSize & 0xff]);
    chunks.push(header, payload.subarray(offset, offset + size));
    offset += size;
  }
...
function buildCheck(checkType) {
  if (checkType === CHECK_NONE) return new Uint8Array(0);
