...
const concatRoot = resolve(fixturesRoot, 'xz-concat');
...
const payload1 = concatBytes([
  encoder.encode('stream-one\n'),
  new Uint8Array([0xe8, 0x00, 0x00, 0x00, 0x00, 0x90, 0x90]),
  encoder.encode('end-one\n')
]);

const payload2 = concatBytes([
  encoder.encode('stream-two\n'),
  new Uint8Array([0x90, 0x90, 0xe8, 0x00, 0x00, 0x00, 0x00, 0x90]),
  encoder.encode('end-two\n')
]);

const startOffset1 = 0;
const startOffset2 = 32;

const stream1 = buildStream(
  [
    buildBlock({
      payload: payload1,
      encoded: encodeX86(payload1, startOffset1),
      filters: [
...
const stream2 = buildStream(
  [
    buildBlock({
      payload: payload2,
      encoded: encodeX86(payload2, startOffset2),
      filters: [
        { id: FILTER_X86, props: uint32le(startOffset2) },
        { id: FILTER_LZMA2, props: new Uint8Array([DICT_PROP]) }
      ],
      checkType: CHECK_CRC32
    })
...
const stream = concatBytes([stream1, stream2]);
const expected = concatBytes([payload1, payload2]);

await mkdir(concatRoot, { recursive: true });
await writeFile(resolve(concatRoot, 'concat-two-streams-bcj.xz'), stream);
await writeFile(resolve(concatRoot, 'concat-two-streams-bcj.bin'), expected);
