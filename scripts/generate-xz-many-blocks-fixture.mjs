...
const manyRoot = resolve(fixturesRoot, 'xz-many-blocks');
...
const payloads = [];
for (let i = 0; i < 5; i += 1) {
  const payload = buildPayload(i, 16000);
...
await writeFile(resolve(manyRoot, 'many-blocks.xz'), stream);
await writeFile(resolve(manyRoot, 'many-blocks.bin'), expected);

function buildPayload(index, size) {
