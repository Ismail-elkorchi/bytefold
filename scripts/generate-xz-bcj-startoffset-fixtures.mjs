...
const block1Payload = concatBytes([
  encoder.encode('bytefold-bcj-startoffset-block-1\n'),
  new Uint8Array([0x90, 0x90, 0x90, 0x90])
]);

const block2Payload = buildBlock2Payload();
const startOffsetBlock2 = block1Payload.length;

const encodedBlock1 = encodeX86(block1Payload, 0);
const encodedBlock2 = encodeX86(block2Payload, startOffsetBlock2);

const blocks = [
  buildBlock({
    payload: block1Payload,
    encoded: encodedBlock1,
    filters: [
  ...
  buildBlock({
    ...
    encoded: encodedBlock2,
    filters: [
      { id: FILTER_X86, props: uint32le(startOffsetBlock2) },
      { id: FILTER_LZMA2, props: new Uint8Array([DICT_PROP]) }
    ],
    checkType: CHECK_CRC32
  })
...
const expected = concatBytes([block1Payload, block2Payload]);

await mkdir(bcjRoot, { recursive: true });
await writeFile(resolve(bcjRoot, 'startoffset-multiblock-x86.xz'), stream);
await writeFile(resolve(bcjRoot, 'startoffset-multiblock-x86.bin'), expected);

function buildBlock2Payload() {
  ...
  payload[10] = 0x90;
  payload[11] = 0x90;
  const label = encoder.encode('block2\n');
  payload.set(label, 12);
  return payload;
}
